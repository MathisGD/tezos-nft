...
def test():
    ...
    locker = Locker(cyber_token.address, admin.address)
    scenario += locker
    ...
    #scenario += cyber_token.burn(address = elon.address, token_id = 5, eth_owner = sp.utils.bytes_of_string('0xadresseth')).run(sender = admin)
    scenario += cyber_token.set_administrator(locker.address).run(sender=admin)
    scenario += locker.deposit(sp.record(address_from = elon.address, 
                                        contract_address =cyber_token.address, 
                                        token_id = 3)).run(sender= elon)
    
    scenario += locker.lockToken(3).run(sender=elon)
    scenario += locker.unlockToken(3).run(sender=elon)
    
    scenario += locker.withdraw(3).run(sender=elon)